@doc raw"""
	write_multi_stage_shadow_capacities(outpath::String, settings_d::Dict)
...
"""
function write_multi_stage_shadow_capacities(outpath::String, settings_d::Dict)
    ...
    df_cap[!, Symbol("StartShadowCap_p1")] = capacities_d[1][!, :StartShadowCap]

    # Store end capacities for all stages
    for p in 1:num_stages
        df_cap[!, Symbol("NewShadowCap_p$p")] = capacities_d[p][!, :NewShadowCap]
    end

    CSV.write(joinpath(outpath, "capacities_shadow_multi_stage.csv"), df_cap)
end
